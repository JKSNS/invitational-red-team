...
def interactive_menu(teams_count: int) -> int:
    while True:
        ...
        if choice == "3":
            ...
            run_script(ROOT_DIR / "actions" / "persistence.py", ["--teams-count", str(teams_count), "--targets", targets])
            continue
        if choice == "4":
            targets = prompt_targets()
            ...
            run_script(
                ROOT_DIR / "actions" / "user_management.py",
        ...
        if choice == "5":
            ...
            run_script(
                ROOT_DIR / "actions" / "service_degradation.py",
                ["--teams-count", str(teams_count), "--targets", targets, "--action", action],
            )
            continue
        if choice == "6":
            ...
            run_script(
                ROOT_DIR / "actions" / "defacement.py",
                ["--teams-count", str(teams_count), "--targets", targets, "--action", action],
            )
            continue
        if choice == "7":
            ...
            run_script(
                ROOT_DIR / "actions" / "chaos_mode.py",
                ["--teams-count", str(teams_count), "--targets", targets, "--action", action],
            )
            continue
        if choice == "Q":
...
def main() -> int:
    ...
    if args.action == "persistence":
        return run_script(ROOT_DIR / "actions" / "persistence.py", ["--teams-count", str(teams_count), "--targets", args.targets])
